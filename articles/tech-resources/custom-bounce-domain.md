...
If you prefer to use the SparkPost UI instead of the API to verify a bounce domain, please use the following instructions. Please note, however, that you cannot specify a default bounce domain via the UI; that must be done via the sending-domains API endpoint.

1. Select Account and then Sending Domains from the main menu.
2. If you have a pre-existing sending domain, you can click the "view settings" link to pull up the sending domain's settings. Otherwise, click the "ADD DOMAIN" orange button in the upper right to being configuring a new domain.
3. Use the "verify DNS records" orange link next to the CNAME record section to verify the domain as a bounce domain.
4. Make sure the domain is verified by the green check mark.
