...
| `bridge_mailfrom` | no | The `mf=` for an auto-generated **fabricated** bridging signature when the new `mf=` is not in the previous signature's `rt=` (§9.2). Required when the prior `rt=` has multiple entries; inferred automatically when it has exactly one. |
...
| `allow_recipe_z` | no | If `true`, accept the `b: {"z": true}` (truncated-body) recipe at sign time. Default `false`. The `-03` spec is internally inconsistent on this recipe shape — the changelog removes it but §11.1 still references it — so the signer refuses to emit it without an explicit opt-in. Set this only if you are interoperating with a verifier that requires the truncated-body recipe and you accept that the shape may be removed from the final spec. |
...
in the previous signature's `rt=` list, §9.2 requires an extra bridging
...
```lua
-- Same mailing-list scenario, bridged via nd= instead of a fabricated hop:
--   i=1 (originator):  d=sender.com        mf=alice@sender.com rt=list@mailing-list.com
--   i=2 (nd= bridge):  d=mailing-list.com  nd=mailing-list.com   (no mf=/rt=)
--   i=3 (primary):     d=mailing-list.com  mf=bounce@mailing-list.com rt=subscriber@recipient.com
local ok, val, info = msys.validate.dkim2.sign(msg, vctx, {
  domain          = "mailing-list.com",
  selector        = "list-2026",
  keyfile         = "/etc/dkim2/mailing-list.com/list-2026.key",
  ...
  rcptto          = "subscriber@recipient.com",
  on_chain_break  = "nd",
  -- bridge identity: a domain in the prior rt= (here the list received at
...
its `Message-Instance` header so the §11.4 chain walk can reconstruct the
original state.
