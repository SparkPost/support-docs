...
1. Create a  [new SparkPost API Key](/docs/getting-started/create-api-keys/) (do a 'Select All' to enable ALL the permissions) - save that API key somewhere safe as SparkPost only displays it once for security reasons.
...
* If Ongage returns the error of 'Over Quota', send a [note to support](/docs/) to request your SparkPost's account sending limits to be increased in order to allow higher message sending rates
